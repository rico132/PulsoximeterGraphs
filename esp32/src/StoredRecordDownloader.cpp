...
public:
  ...
  bool readReport(uint8_t report[64]) override {
    // esp_rom_printf, not Serial: a task watchdog panic during a real-device
    // test showed usbTask genuinely *blocked* (CPU 1 idle, not running) at a
    // fixed, reproducible report count that doesn't recover on retry, with
    // BLE and core placement both ruled out as factors. The watchdog panic
    // itself (Tasks currently running: CPU0 IDLE0, CPU1 IDLE1, a clean
    // task_wdt abort) confirms neither core was actually frozen — ticks and
    // the scheduler were healthy throughout, which argues against
    // xQueueReceive()'s own bounded wait being what's stuck (FreeRTOS's
    // tick-driven delayed-task list would still wake it). esp_rom_printf
    // busy-waits directly on the UART hardware, bypassing the driver/ISR
    // entirely, so bracketing host_.readReport() with it on both sides (see
    // the second esp_rom_printf right after the call below) narrows down
    // whether the still-untested suspect — the Serial.printf() a few lines
    ...
    // Real-device testing has now shown the "RR#" print above itself
    // stopping (no next RR# ever appears) at a fixed, reproducible report
    // count that never recovers on retry — but that print only brackets
    // *entry* to host_.readReport(); it was never proof the call itself is
    // what's stuck, since the very next line below this one is a
    // Serial.printf(), and Serial's UART write path (unlike esp_rom_printf,
    // which busy-waits directly on the hardware) can itself block
    // indefinitely if its TX ring buffer fills and the draining interrupt is
    // ever starved — which was never actually ruled out for *this specific*
    // line, only for the RR# heartbeat itself. This second esp_rom_printf,
    // right after host_.readReport() returns and before any Serial call,
    // closes that gap: if "RR#N returned" is the last thing seen, the block
